...
- **Zellij Keybindings**: Keep Zellij default Alt pass-through shortcuts as static `package.json` keybinding contributions scoped to `terminalFocus && config.tmuxWorktree.multiplexer == 'zellij'`. Apply VS Code terminal setting overrides from `src/utils/zellijTerminalSettings.ts` only while the Zellij backend is active, then restore extension-applied global values when switching away.
