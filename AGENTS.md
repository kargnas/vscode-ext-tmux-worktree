...
- **VS Code xterm.js Clipboard Addon Breaks UTF-8 (Korean/Chinese/Emoji Mojibake)**: VS Code's bundled `@xterm/addon-clipboard` (`node_modules/@xterm/addon-clipboard/lib/addon-clipboard.js`) decodes OSC52 base64 via `atob()` and encodes via `btoa()`. Both are Latin-1 only; multi-byte UTF-8 byte sequences get reinterpreted as per-byte char codes (e.g. `천` UTF-8 `EC B2 9C` → clipboard string `ì²` + control char). Pasting later re-encodes that as UTF-8 → classic mojibake (`천우` → `ì²ì°`, `→` → `â`). tmux itself emits OSC52 with correct UTF-8 bytes; the corruption happens entirely inside VS Code. Workaround: route tmux's default copy bindings through a real OS clipboard tool by setting `copy-command pbcopy|wl-copy|xclip|xsel|clip.exe` and `set-clipboard off` so the default `copy-pipe-and-cancel` (no shell-command arg) feeds the selection into the OS tool instead of going through xterm.js. End-to-end verified: mouse drag end, Enter, double/triple click in copy-mode all preserve UTF-8 with this path. Keep `allow-passthrough on` so OpenCode/Claude Code TUI OSC52 still flows through (those bytes still hit the broken handler, but disabling passthrough would silently drop their copy attempts). Headless server fallback: when no OS clipboard tool is available, keep `set-clipboard on` + `terminal-features/overrides ...:clipboard` so OSC52 still happens (with mojibake for multi-byte).
