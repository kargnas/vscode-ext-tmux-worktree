...
func (m Model) View() string {
	...
	if m.state == stateProjectList {
		...
		if len(m.list.Items()) == 0 {
			header := lipgloss.JoinVertical(lipgloss.Left, tabHeader, filterLine)
			var content string
			if m.activeTab == TabActiveSessions {
				content = EmptyStyle.Render("No active sessions") + "\n\n" +
					HintStyle.Render("Press Tab to switch to All Projects")
			} else if m.dirtyFilter {
				content = EmptyStyle.Render("No dirty projects") + "\n\n" +
					HintStyle.Render("Press Space to disable filter")
			} else {
				content = EmptyStyle.Render("⚠️  No projects found!") + "\n\n" +
...
func (m Model) renderTabs() string {
	...
	// JoinHorizontal returns a single line string (height depends on border)
	// We DO NOT add \n here because JoinVertical will handle it
